...
MyGame::MyGame() : AbstractGame(), score(1), lives(2), numKeys(2500), gameWon(false), box(10, 6, 30, 30) {
	TTF_Font * font = ResourceManager::loadFont("res/fonts/brownie.ttf", 35);
	gfx->useFont(font);
    ...
    for (int i = 0; i < numKeys; i++) {
		...
		particle->color = getRandomColor(250, 255);
    }
}
...
void MyGame::handleKeyEvents() {
	...
	if (eventSystem->isPressed(Key::A)) { 
		for (auto key : Particles)
			if (key->isAlive) 
				key->pos = Point2(getRandom(50, 400), getRandom(60, 400));
	} 
